...
def _preview_comp_reroll(
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"**Cost:** ✨ {item['cost']:,} dream pts\n\n"
            f"After a `/randomcomp` result is posted, use `/useitem` to reroll "
            f"just your own slot. You'll get a new random role from whatever's "
            f"left in the pool — you can't reroll into a role already taken by someone else. "
            f"Each reroll is tracked in the weekly recap.\n\n"
            f"⚠️ Consumed on use. Can own multiple.\n\n"
            f"{_afford_line(item, balance, already_owned, consumable=True)}"
        ),
        color=0x9C27B0,

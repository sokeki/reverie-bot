        ...
        for guild in self.bot.guilds:
            ...
            for match_id, detected_accounts in by_match.items():
                ...
                if match_id not in matches_fetched:
                    # Fetch full v2 match directly — includes mmr_change_to_last_game per player
                    await self._cache_full_match(match_id)
                    cached = await self.bot.val_match_cache_col.find_one({"match_id": match_id})
                    match_data = cached["data"] if cached else await self._get_match_details(
        ...
        if rr_change == 0:
            ...
            if history:
                ...
                if entry:
                    ...
                    print(f"[Val Tracker] MMR history: match not found for {name}#{tag}, rr_change unknown")

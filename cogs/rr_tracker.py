...
async def _get_agent_icon(session: aiohttp.ClientSession, agent_name: str) -> str | None:
...
class RRTracker(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self._recently_posted: set[str] = set()  # guard against duplicate posts — stores 'puuid:match_id'
    ...
    async def _get_mmr(self, name: str, tag: str, region: str = "eu") -> dict | None | str:
        session = await self._get_session()
        url = f"{API_BASE}/valorant/v3/mmr/{region}/{PLATFORM}/{quote(name)}/{quote(tag)}"
        try:
            async with session.get(url) as resp:
                if resp.status == 429:
    ...
    async def _get_matches(self, name: str, tag: str, count: int = 1, region: str = "eu") -> list:
        session = await self._get_session()
        ...
        try:
            async with session.get(url) as resp:
                ...
                if resp.status != 200:
                    ...
                    print(f"[Val Tracker] Matches API {resp.status} for {name}#{tag}: {text[:200]}")
                    return []
                data = await resp.json()
                return data.get("data", [])
    ...
    async def _get_match_details(self, name: str, tag: str, region: str = "eu") -> dict | None:
            ...
            try:
                async with session.get(url) as resp:
                    if resp.status == 200:
                        ...
                        print(f"[Val Tracker] Cached {match_id[:8]}... ({len(rounds)} rounds)")
                        break
                    elif resp.status == 429:
                        wait = 10 * (attempt + 1)
                        print(f"[Val Tracker] Rate limited caching {match_id[:8]}..., waiting {wait}s")
                        await asyncio.sleep(wait)
                    else:
                        print(f"[Val Tracker] Cache fetch {resp.status} for {match_id[:8]}...")
                        break
            except Exception as e:
    ...
    async def _get_full_matches(self, matches: list) -> list:
        ...
        for match in matches:
            match_id = match.get("metadata", {}).get("matchid") or match.get("metadata", {}).get("match_id")
            if not match_id:
                continue
                ...
                try:
                    async with session.get(url) as resp:
                        if resp.status == 200:
                            ...
                            full_match = data.get("data", data)
                            rounds     = full_match.get("rounds", [])
                            print(f"[Val Tracker] Fetched & cached {match_id[:8]}... ({len(rounds)} rounds)")
                            await self.bot.val_match_cache_col.update_one(
                                {"match_id": match_id},
                                {"$set": {
                                    "data":       full_match,
                            ...
                            wait = 10 * (attempt + 1)
                            print(f"[Val Tracker] Rate limited on match fetch, waiting {wait}s...")
                            await asyncio.sleep(wait)
                        else:
                            print(f"[Val Tracker] Match fetch {resp.status} for {match_id[:8]}...")
                            break
                except Exception as e:
    ...
    async def _trim_match_cache(self, puuid: str) -> None:
        ...
        docs = await self.bot.val_match_cache_col.find(
            {"$or": [{"puuid": puuid}, {"puuids": puuid}]}
        ).sort("cached_at", -1).skip(20).to_list(length=1000)
        if docs:
    ...
    @app_commands.command(name="registerriot", description="Add a Riot account to server tracking (Valorant RR + TFT)")
    ...
    async def registerriot(self, interaction: discord.Interaction, username: str, region: str = "EUW"):
        if "#" not in username:
        ...
        riot_puuid = None
        try:
            routing = {"eu": "europe", "na": "americas", "ap": "asia", "kr": "asia", "br": "americas", "latam": "americas"}.get(val_region, "europe")
            async with (await self._get_session()).get(
    ...
    @app_commands.command(name="unregisterriot", description="Remove a Riot account from server tracking")
    ...
    @app_commands.command(name="setvalchannel", description="[Admin] Set the channel for Valorant RR tracking updates")
    @app_commands.describe(channel="Channel to post RR updates in")
    @app_commands.default_permissions(administrator=True)
    async def setvalchannel(self, interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    @app_commands.command(name="valleaderboard", description="See the Valorant RR leaderboard for registered players")
    async def valleaderboard(self, interaction: discord.Interaction):
        ...
        for account in accounts:
            mmr = await self._get_mmr(account["val_name"], account["val_tag"], account.get("val_region", "eu"))
        ...
        if not rows:
            await interaction.followup.send("*couldn't fetch RR data right now.*", ephemeral=True)
            return
        ...
        for i, row in enumerate(rows):
            ...
            lines.append(f"{medal} **{row['name']}** - {row['tier']} {row['rr']} RR{change}")
    ...
    @app_commands.command(name="valtrackertest", description="[Admin] Test the Valorant API for a specific account")
    ...
    @app_commands.command(name="valtrackerstatus", description="[Admin] Check if the Valorant tracker is running")
    @app_commands.default_permissions(administrator=True)
    async def valtrackerstatus(self, interaction: discord.Interaction):
        accounts = await self.bot.riot_accounts_col.find(
            {"guild_id": interaction.guild_id}
        ).to_list(length=100)
        settings = await self.bot.settings_col.find_one({"guild_id": interaction.guild_id})
        ...
        if accounts:
            for a in accounts:
                lines.append(f"- {a['val_name']}#{a['val_tag']} ({a.get('val_region', 'eu').upper()}) - last match ID: `{a.get('last_match_id') or 'null'}`")

        await interaction.response.send_message("\n".join(lines), ephemeral=True)

    # ── /valstats ─────────────────────────────────────────────────────────────

    @app_commands.command(name="valstats", description="Show Valorant stats for any player")
    @app_commands.describe(
        username="Riot ID including tag, e.g. Name#EUW",
        region="Server region",
    ...
    async def valstats(self, interaction: discord.Interaction, username: str, region: str = "EUW", detail: str = None):
        if "#" not in username:
            await interaction.response.send_message("⚠️ Include the tag, e.g. `Name#EUW`.", ephemeral=True)
            return
        ...
        mmr = await self._get_mmr(name, tag, val_region)
        if mmr == "rate_limited":
            await interaction.followup.send("⚠️ The API is currently rate limited - please try again in a minute.", ephemeral=True)
            return
        if not mmr:
            await interaction.followup.send(f"⚠️ Couldn't find **{username}** on **{region}**.")
            return
        ...
        if mmr_v2:
            ...
            if by_season:
                ...
                current_act = next(
                    (data for act, data in sorted(by_season.items(), reverse=True)
                     if not data.get("error", False) and data.get("number_of_games", 0) > 0),
                    None
                )
                if current_act:
        ...
        for match in fresh:
            ...
            if match_id and puuid:
                ...
                if isinstance(raw_p_f, list):
                    ...
                    all_p_f = []
                match_puuids = [p.get("puuid") for p in all_p_f if isinstance(p, dict) and p.get("puuid")]
                exists = await self.bot.val_match_cache_col.find_one({"match_id": match_id})
                if not exists:
        ...
        matches = []
        if puuid:
            cached_docs = await self.bot.val_match_cache_col.find(
                {"$or": [{"puuid": puuid}, {"puuids": puuid}]}
            ...
            print(f"[Val Tracker] valstats cache: {len(cached_docs)} docs for puuid={puuid[:8] if puuid else None}, has_rounds counts: {sum(1 for d in cached_docs if d.get('has_rounds'))}/{len(cached_docs)}")
            if cached_docs:
        ...
        for match in matches:
            ...
            raw   = match.get("players", [])
            all_p = raw if isinstance(raw, list) else raw.get("all_players", [])
            player = next((p for p in all_p if isinstance(p, dict) and p.get("puuid") == puuid), None)
            if not player:
                continue
            s = player.get("stats", {})
        ...
        if not detail:
            ...
            embed.add_field(name="Season",  value=f"**{s_wins}W {s_losses}L** ({s_wr}%)\n{s_games} games", inline=True)
            embed.add_field(name="\u200b", value="\u200b",                             inline=True)
            embed.add_field(name="\u200b", value="\u200b",                             inline=True)
            ...
            embed.set_footer(text=f"Last {games_counted} competitive games  •  Reverie  •  {interaction.guild.name}")
            ...
            for match in full_matches:
                ...
                team_map  = {p["puuid"]: p.get("team", "").lower() for p in all_p if isinstance(p, dict)}
                ...
                for rnd_idx, rnd in enumerate(rounds):
                    ...
                    rnd_kills  = sorted(kills_by_round.get(rnd_idx, []), key=lambda k: k.get("time_in_round_in_ms", 0))
            ...
            embed.add_field(name="\u200b",         value="\u200b",                          inline=True)
            embed.add_field(name="\u200b",         value="\u200b",                          inline=True)
            embed.add_field(name="Clutch %",        value=f"**{clutch_pct}%**\n{clutch_wins}/{clutch_opps}", inline=True)
            embed.add_field(name="First Bloods/g",  value=f"**{fb_pg}**\n{first_bloods} total", inline=True)
            embed.add_field(name="\u200b",         value="\u200b",                          inline=True)
            embed.set_footer(text=f"Last {games_counted} competitive games  •  Reverie  •  {interaction.guild.name}")
            ...
            embed.add_field(name="\u200b",         value="\u200b",                          inline=True)
            embed.add_field(name="\u200b",         value="\u200b",                          inline=True)
            embed.add_field(name="C Cast", value=f"**{round(total_c/g,1)}/g**\n{total_c} total", inline=True)
            embed.add_field(name="Q Cast", value=f"**{round(total_q/g,1)}/g**\n{total_q} total", inline=True)
            embed.add_field(name="E Cast", value=f"**{round(total_e/g,1)}/g**\n{total_e} total", inline=True)
            embed.add_field(name="X Cast", value=f"**{round(total_x/g,1)}/g**\n{total_x} total", inline=True)
            embed.add_field(name="\u200b",         value="\u200b",                          inline=True)
            embed.add_field(name="\u200b",         value="\u200b",                          inline=True)
            embed.set_footer(text=f"Last {games_counted} competitive games  •  Reverie  •  {interaction.guild.name}")
            ...
            for match_idx, match in enumerate(full_matches):
                ...
                player = next((p for p in all_p if isinstance(p, dict) and p.get("puuid") == puuid), None)
                if not player:
                    print(f"[Val Tracker] Behaviour: player not found in match {match_idx}")
                    continue
                rounds_played = match.get("metadata", {}).get("rounds_played", 0)
            ...
            embed.add_field(name="\u200b",         value="\u200b",                          inline=True)
            embed.add_field(name="\u200b",         value="\u200b",                          inline=True)
            embed.add_field(name="AFK Rounds/g",   value=f"**{round(total_afk/g,1)}**\n{int(total_afk)}/{total_rounds_beh} rounds",   inline=True)
            embed.add_field(name="Spawn Rounds/g", value=f"**{round(total_spawn/g,1)}**\n{int(total_spawn)}/{total_rounds_beh} rounds", inline=True)
            embed.add_field(name="\u200b",         value="\u200b",                            inline=True)
            embed.add_field(name="FF Outgoing/g",  value=f"**{round(ff_out/g,1)}**\n{int(ff_out)} total",             inline=True)
            embed.add_field(name="FF Incoming/g",  value=f"**{round(ff_in/g,1)}**\n{int(ff_in)} total",               inline=True)
            embed.add_field(name="\u200b",         value="\u200b",                            inline=True)
            embed.set_footer(text=f"Last {games_counted} competitive games  •  Reverie  •  {interaction.guild.name}")
            ...
            for match in matches:
                ...
                won    = (player.get("team_id") or player.get("team", "")).lower() == _winning_team(match).lower()
                acs    = round(s.get("score", 0) / max(rounds, 1))
                if agent not in agent_stats:
                    agent_stats[agent] = {"games": 0, "wins": 0, "kills": 0, "deaths": 0, "acs": 0}
                ...
                ag["acs"]    += acs
            sorted_agents = sorted(agent_stats.items(), key=lambda x: x[1]["games"], reverse=True)
            lines = []
            ...
            for match in matches:
                ...
                map_name = raw_map if isinstance(raw_map, str) else raw_map.get("name", "Unknown")
                raw   = match.get("players", [])
                all_p = raw if isinstance(raw, list) else raw.get("all_players", [])
                player = next((p for p in all_p if isinstance(p, dict) and p.get("puuid") == puuid), None)
                if not player:
            ...
            sorted_maps = sorted(map_stats.items(), key=lambda x: x[1]["games"], reverse=True)
            lines = []
    ...
    @app_commands.command(name="footshot", description="Check a player's shot accuracy across their last 10 competitive games")
    ...
    async def footshot(self, interaction: discord.Interaction, username: str):
        ...
        if not matches:
            await interaction.followup.send(
                f"*no recent competitive games found for **{username}**.*", ephemeral=True
            )
            return
        ...
        for match in matches:
            ...
            all_players = raw_players if isinstance(raw_players, list) else raw_players.get("all_players", [])
        ...
        if not puuid:
            await interaction.followup.send(
                f"⚠️ Couldn't identify **{username}** in their match history.", ephemeral=True
            )
            return
        ...
        embed.add_field(name="Headshots", value=f"**{total_hs}**\n{hs_pct}%", inline=True)
        embed.add_field(name="Bodyshots", value=f"**{total_bs}**\n{bs_pct}%", inline=True)
        embed.add_field(name="Legshots",  value=f"**{total_ls}**\n{ls_pct}%", inline=True)
        embed.set_footer(text=f"Last {games_counted} competitive games  •  Reverie  •  {interaction.guild.name}")
        await interaction.followup.send(embed=embed)

        # ── /scoreboard ───────────────────────────────────────────────────────────

    @app_commands.command(name="scoreboard", description="Show the scoreboard for a match. Provide a match ID or a username to use their latest game.")
    @app_commands.describe(
        match_id="Match ID from the RR update footer",
        username="Valorant username and tag to use their latest game, e.g. Name#EUW",
    )
    async def scoreboard(self, interaction: discord.Interaction, match_id: str = None, username: str = None):
        if not match_id and not username:
            await interaction.response.send_message("⚠️ Provide either a match ID or a username.", ephemeral=True)
            return
        ...
        if match_id:
            ...
            try:
                async with session.get(url) as resp:
                    if resp.status != 200:
                        await interaction.followup.send(f"⚠️ Couldn't find match `{match_id}`.", ephemeral=True)
                        return
                    data = await resp.json()
            ...
            if not matches:
                await interaction.followup.send(f"⚠️ No recent matches found for **{username}**.", ephemeral=True)
                return
            match = matches[0]
    ...
    def _build_scoreboard_embed(self, match: dict, guild: discord.Guild) -> discord.Embed:
        ...
        def _short_rank(rank: str) -> str:
            short = {"Iron": "Iron", "Bronze": "Brnz", "Silver": "Silv", "Gold": "Gold",
                     "Platinum": "Plat", "Diamond": "Dia", "Ascendant": "Asc",
                     "Immortal": "Imm", "Radiant": "Rad"}
        ...
        def build_table(players: list) -> str:
            ...
            for p in players:
                ...
                rows.append(f"{pname:<12} {agent:<9} {rank:<7} {k:>3} {d:>3} {a:>3} {acs:>4} {hs_pct:>4}")
            return "```\n" + "\n".join(rows) + "\n```"
    ...
    async def scoreboard_prefix(self, ctx: commands.Context):
        ...
        if not match_id:
            await ctx.reply("⚠️ Reply to an RR update embed to pull up the scoreboard, or use `/scoreboard match_id:...`.")
            return
        ...
        for guild in self.bot.guilds:
            ...
            for account in accounts:
                try:
                    ...
                    if new_game:
                        new_games.append((account, new_game[0], new_game[1], new_game[2], new_game[3]))
                except Exception as e:
                    print(f"[Val Tracker] Error polling {account.get('val_name')}#{account.get('val_tag')}: {e}")
                await asyncio.sleep(1)
            ...
            for match_id, detected_accounts in by_match.items():
                ...
                if match_data:
                    ...
                    all_players = raw_players if isinstance(raw_players, list) else raw_players.get("all_players", [])
                    all_puuids_in_match = {p.get("puuid") for p in all_players if isinstance(p, dict)}
                    match_game_start = match_data.get("metadata", {}).get("game_start", 0)
                print(f"[Val Tracker] Match scan: {len(all_puuids_in_match)} players, {len(accounts)} registered accounts")

                # Puuids directly detected in phase 1 for this match
                detected_puuids = {a.get("puuid", "") for a, mid, *_ in new_games if mid == match_id}
                ...
                for account in accounts:
                    ...
                    await self.bot.riot_accounts_col.update_one(
                        {"_id": account["_id"]},
                        {"$set": {"last_match_id": match_id, "last_game_start": match_game_start}},
                    )
                    ...
                    if puuid not in detected_puuids:
                        print(f"[Val Tracker] Found in match: {account.get('val_name')}#{account.get('val_tag')}: {match_id}")
                    accounts_to_post.append(account)

                # Build lookup of rr data from new_games detections
                rr_data = {a.get("puuid", ""): (rc, r, t) for a, mid, rc, r, t in new_games if mid == match_id}
                for account in accounts_to_post:
                    try:
                        puuid_ = account.get("puuid", "")
                        rc, r, t = rr_data.get(puuid_, (0, 0, ""))
                        await self._post_new_game(account, match_id, channel, guild, match_data, rr_change=rc, rr=r, tier_from_history=t)
                    except Exception as e:
                        import traceback
                        print(f"[Val Tracker] Error posting {account.get('val_name')}#{account.get('val_tag')}: {e}")
                        traceback.print_exc()
    ...
    async def _detect_new_game(self, account: dict) -> tuple | None:
        ...
        print(f"[Val Tracker] New game detected for {name}#{tag}: {match_id} ({rr_change:+d}RR)")
        return (match_id, rr_change, rr, tier)

    async def _post_new_game(self, account: dict, match_id: str, channel: discord.TextChannel, guild: discord.Guild, latest: dict = None, rr_change: int = 0, rr: int = 0, tier_from_history: str = ""):
        ...
        if rr_change == 0 and latest:
            ...
            player_entry = next((p for p in all_p_m if isinstance(p, dict) and p.get("puuid") == puuid_acc), None)
            if player_entry:
                rr_change = player_entry.get("mmr_change_to_last_game", 0) or 0
                print(f"[Val Tracker] RR change from match data for {name}#{tag}: {rr_change:+d}")

        # Fallback: retry history until match appears (Henrik cache lag)
        if rr_change == 0:
            for attempt in range(12):  # up to 3 minutes (12 x 15s)
                await asyncio.sleep(15)
                print(f"[Val Tracker] Fetching MMR history for {name}#{tag} (attempt {attempt + 1}/12)...")
                ...
                if history:
                    ...
                    if entry:
                        ...
                        print(f"[Val Tracker] MMR history OK for {name}#{tag}: rr_change={rr_change:+d}")
                        break
                    else:
                        print(f"[Val Tracker] MMR history: not yet updated for {name}#{tag} (history[0]={history[0].get('match_id', '?')[:8]}...)")
            else:
                print(f"[Val Tracker] MMR history: gave up after 3 minutes for {name}#{tag}")
        ...
        if latest is None:
            ...
            await asyncio.sleep(1)
            latest = await self._get_match_details(name, tag, account.get("val_region", "eu"))
            print(f"[Val Tracker] Match details {'OK' if latest else 'FAILED'} for {name}#{tag}")
        ...
        if latest:
            ...
            if player:
                ...
                agent   = player.get("agent", {}).get("name") or player.get("character", "Unknown")
                won     = (player.get("team_id") or player.get("team", "")).lower() == _winning_team(latest).lower()
        ...
        is_placement = tier_name in ("Unrated", "Unranked", "") or rr == 0 and rr_change == 0
        result_str   = "WIN" if won else "LOSS"
        embed_colour = COLOUR_MAIN if won else 0x8b4a4a

        session        = await self._get_session()
        card_url       = f"https://media.valorant-api.com/playercards/{player_card_id}/smallart.png" if player_card_id else None
        ...
        if latest and match_id:
            try:
                ...
                if not exists:
                    ...
                    raw_p    = latest.get("players", [])
                    all_p_c  = raw_p if isinstance(raw_p, list) else raw_p.get("all_players", [])
                    all_puuids = [p.get("puuid") for p in all_p_c if isinstance(p, dict) and p.get("puuid")]
                    await self.bot.val_match_cache_col.insert_one({
                        "match_id":   match_id,
                        "puuid":      account.get("puuid", ""),
                        "puuids":     all_puuids,
    ...
    async def daily_summary_task(self):
        ...
        print(f"[Daily Summary] Midnight window hit at {now.strftime('%H:%M')} UTC — date: {today}")
    ...
    async def _post_daily_summary(self, guild: discord.Guild):
        ...
        for player_key, player_games in sorted(
            ...
            rank_str   = f"{start_tier} {start_rr}rr → {end_tier} {end_rr}rr" if start_tier != end_tier else f"{end_tier} {start_rr}rr → {end_rr}rr"
